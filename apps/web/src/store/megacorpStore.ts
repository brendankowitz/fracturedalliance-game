...
export const useMegacorpStore = create<MegacorpState>((set, get) => ({
  ...
  addRep: (delta) => {
    ...
    try { localStorage.setItem(REP_STORAGE_KEY, String(next)); } catch { /* ignore */ }
    set({ megacorpRep: next });
  },
}));

export const THEME_COLORS: Record<HudTheme, { bg: string; border: string; text: string; accent: string }> = {
